...
def main():
    ...
    tokenizer = LlamaTokenizer.from_pretrained(args.model_name_or_path_finetune)
    tokenizer.pad_token = tokenizer.eos_token

    # model_baseline = get_model(config, args.model_name_or_path_baseline,
                            #    tokenizer)
    model_fintuned = get_model(AutoConfig.from_pretrained(args.model_name_or_path_finetune), args.model_name_or_path_finetune,
                               tokenizer)
    ...
    if args.language == "English":
        ...
        prompts = [
            ...
            "Human: 写一首关于一只聪明的臭猪的短诗。 Assistant:",
    ...
    prompt_eval(args, model_fintuned, tokenizer, device,
                prompts)
